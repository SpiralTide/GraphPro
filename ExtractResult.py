...
model.load_state_dict(torch.load('pretrained/pretrained_G_200.pth'))

# Test阶段
model.eval()

for i, data in enumerate(train_dataloader):
    # 我们现在针对每一个长度为480的batch计算其对应的acc
    ...
    if i == 2:
        saved_y_speed = y_speed.detach().cpu().numpy()
        saved_pred_speed = pred_speed.detach().cpu().numpy()

        np.save('y_speed', saved_y_speed)
        np.save('pred_speed', saved_pred_speed)
